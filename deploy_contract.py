...
"""
    ...
    def verify_prerequisites(self):
        ...
        # Ensure npm global bin is in PATH for Python
        import sys
        npm_bin_path = r"C:\Users\arin\AppData\Roaming\npm"
        if npm_bin_path not in os.environ["PATH"]:
            os.environ["PATH"] += os.pathsep + npm_bin_path

        try:
            hardhat_cmd = os.path.join(npm_bin_path, "hardhat.cmd")
            if not os.path.exists(hardhat_cmd):
                hardhat_cmd = "hardhat"  # fallback if not found
            result = subprocess.run([hardhat_cmd, "--version"], capture_output=True, text=True)
            print("Hardhat check output:", result.stdout, result.stderr)
            if result.returncode == 0 and result.stdout.strip():
                hardhat_available = True
            else:
                hardhat_available = False
        except Exception as e:
            print("Hardhat check exception:", e)
            hardhat_available = False
